...
class ProductControllerTestIT extends AbstractIntegrationTest {
  ...
  @BeforeEach
  void setUp() {
    reset(findProductUserUseCase, productResponseMapper, findProductByTerm, findProductAllUseCase, createProductUseCase, tokenService,
        addReviewToProduct, addProductToMyVanityUseCase, deleteProductFromUserVanityUseCase);
  }
  ...
  void shouldAddReviewToProduct() throws Exception {
    ...
    ProductUserRelation relation = ProductUserRelation.create(productId, entityId);
    Set<ProductUserRelation> relations = new HashSet<>();
    relations.add(relation);

    Review review = Review.createWithExistingId(reviewId, relation.getId(), ReviewDetails.create(5, "Great product"));
